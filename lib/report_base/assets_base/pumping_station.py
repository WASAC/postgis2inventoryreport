...
class PumpingStations(AssetsBase):
    ...
    def get_assets_info(self, db):
        ...
        query += "    b.status,  "
        query += "    a.observation,  "
        query += "    CASE WHEN a.water_meter = 1 THEN 'YES' ELSE 'NO' END as has_water_meter,"
        query += "    a.meter_installation_date,"
        query += "    CASE WHEN a.installation_antihummer = 1 THEN 'YES' ELSE 'NO' END as installation_antihummer,  "
